...
0. **Sync overviews (Step 0):** Run the `sync-overviews` skill (`@.claude/skills/sync-overviews/skill.md`) before reading any project data. This ensures `_overview.md` files reflect the latest committed changes. If no projects were updated, proceed silently. If any were updated, include the sync report in the brief as a collapsed `<details>` block.
...
### 📌 Project pulse
- **Korvex** — {{one line, e.g. "waiting on Brisas content delivery"}}
- **Brisas del Golfo** — {{...}}
- **University** — {{...}}
```

...
description: Spin up a new Korvex client — create a person note in 07-people/clients/, a project file in 02-projects/korvex/clients/, prompt Eduardo for the essentials (business type, services, contact, start date, channel), and produce a client brief. Use when Eduardo says "onboard client [name]", "nuevo cliente [nombre]", or "new client [name]".
---

# Client Onboarding

Every new Korvex client needs the same scaffolding: a relationship note (who they are), a project file (what we're delivering), a brief (the at-a-glance summary), and a clear comms channel. This skill creates all of that in one pass and asks Eduardo only for the things that can't be inferred.

## Workflow

1. **Parse the client name** from Eduardo's request. Generate a kebab-case slug (e.g., "Restaurante La Brisa" → `restaurante-la-brisa`).
2. **Check for collisions**: Glob `07-people/clients/<slug>*.md` and `02-projects/korvex/clients/<slug>*.md`. If files exist, ask Eduardo if this is a new engagement or a continuation.
...
6. **Create the project file** at `02-projects/korvex/clients/<slug>.md` (template below).
7. **Update `02-projects/korvex/_overview.md`** — add the new client to the active client list (Edit, don't rewrite).
8. **Update `00-inbox/_ACTIVE-PROJECTS.md`** if Korvex's status line should change (e.g., new client takes Eduardo from "1 active client" to "2 active clients").
9. **Generate and output a client brief** (format below) for Eduardo to skim or share.
...
## Project file template (02-projects/korvex/clients/<slug>.md)
...
- [[02-projects/korvex/clients/<slug>]] — proyecto activo
...
- Default language is Spanish — Korvex clients are local. Only switch to English if Eduardo explicitly indicates the client is English-speaking.
...
- Don't auto-send anything (no email, no WhatsApp). The brief is for Eduardo, not the client.

## Example invocation

```
Eduardo: nuevo cliente Restaurante La Brisa
...
→ Write 02-projects/korvex/clients/restaurante-la-brisa.md
→ Edit 02-projects/korvex/_overview.md → add to client list
→ Echo brief
```

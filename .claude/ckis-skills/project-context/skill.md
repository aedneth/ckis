...
description: Compile a compact context brief for a project — read its _overview.md, scan recent daily notes for mentions, gather all tagged notes, and surface current status, open decisions, recent progress, and blockers. Use when [OWNER] says "project context [name]", "contexto del proyecto [nombre]", or at the start of any coding session on [YOUR_PROJECT] / Brisas / [ARCHIVED_PROJECT] / University.
---
...
1. **Identify the project.** Match [OWNER]'s input against `02-projects/` folders. Common shortcuts:
   - "[your-project]" → `02-projects/[your-project]/`
   - "[your-project-tag]" / "[client-site]" → `02-projects/[client-site]/`
   - "[archived-project]" → `02-projects/[archived-project]/`
...
4. **Scan recent daily notes** for project mentions:
   ...
   - Grep them for the project name and any associated tags (e.g., `#[your-project]`, `#your-project-tag`)
...
6. **Dev Brain integration (optional — runs if `~/Documents/Dev Brain/` exists).**
   ...
   - If it exists, resolve the Dev Brain slug for the project:
     ...
     - `brisas` / `[client-site]` → `[client-site]`
...
10. **Optionally log the session start** to `01-daily/logs/{{YYYY-MM-DD-HHMM}}-{{project}}-session-start.md` if [OWNER] confirms (or if invoked at the start of an actual coding session — ask).
...
- Bilingual: [YOUR_PROJECT]/Brisas notes are mostly Spanish → write the brief in Spanish. University/personal-brand may be English. Match the source.
- Never modify project files. The skill is read-only on `02-projects/`.
- "Suggested next action" must be concrete (not "continue working on [YOUR_PROJECT]"). One thing, doable today.
- If `_overview.md` doesn't exist, say so and offer to create one from the template.
- The session-start log is opt-in — don't write it unless invoked in a session-start context or [OWNER] says so.

## Example invocation

```
[OWNER]: project context [your-project]
→ Read 02-projects/[client-site]/_overview.md and all sub-files
→ Glob 01-daily/, take last 14, Grep for "[your-project-tag]" and "#your-project-tag"
→ Grep 03-knowledge/ for #your-project-tag tag
